...
| Advisor Call QA Coach | Score a real service-advisor phone call against a 5-dimension rubric, identify the moment-of-inflection turn that decided the outcome, surface missed upsells and weak objection handling, and produce a one-thing-to-rehearse-next-time coaching note the manager can use in a 1:1. | ~15 min/call audit + measurable advisor lift |
...
| Service Advisor Roleplay Coach | Run a structured roleplay rehearsal between a service advisor and an AI-played customer persona, then score the transcript against a shop-defined rubric and produce a coaching note the manager can use in a 1:1 review. | ~30 min/practice session + measurable advisor-skill lift |
...
| Vehicle Care Plan Builder | Turn a customer's open declined work, scheduled preventive-maintenance intervals, and known wear items into a single forward-looking 12-month vehicle care plan — a month-by-month roadmap that sequences the work by safety priority, spreads cost across the year, sets clear booking windows, and gives the customer something to keep on the fridge or in their glovebox. | ~25 min/customer + higher declined-work conversion + retention lift |
...
| AI Tire Inspection Customer Recap & Upsell | Translate raw tire-inspection data — from AI vision tools like Anyline TireBuddy, TyreSwift, UVeye, or a manual tread-gauge + visual check — into a clear, jargon-free customer explanation that tells the vehicle owner exactly what each corner of their car is showing, how urgent it is, and whether they need new tires, an alignment, both, or just a watch-and-rotate. | ~8 min/vehicle + higher tire & alignment authorization rate |
...
| EV/Hybrid Battery Health Customer Recap | Translate raw battery health test results — from tools like BATTSCAN, Dr.EV, AVILOO, GreenTech, or a manual multi-meter cell test — into a clear, jargon-free customer explanation that tells the hybrid or EV owner what their battery is doing right now, how urgent any issues are, and exactly what they should do next. | ~10 min/vehicle |
| Fake Review & Extortion Dispute Documenter | When the shop is hit with a review that is fake, planted by a competitor, off-topic, or part of a "pay-us-or-the-1-stars-stay" extortion run, this skill builds the documented case to get it removed — a platform flag/appeal written to the specific policy it violates, the merchant-extortion report content when money is being demanded, and a calm public holding reply to post while removal is pending. | ~25 min/disputed review |
...
| AI Customer-Contact Compliance Reviewer | Take any **outbound customer contact the shop is about to send — or any AI voice/chat agent script it is about to deploy — and review it before it goes live** against the four things that actually get shops sued or fined: (1) whether the message is *informational* or *marketing*, and therefore what tier of consent it needs; (2) whether the shop can prove it *has* that consent for the numbers on the list; (3) whether the required disclosures are present (AI identity, call recording, sender identification, opt-out); and (4) whether the opt-out path actually works and gets honored. | ~20 min per campaign/script + materially reduced per-message penalty exposure |
| Chargeback Defense Documenter | Organize all available repair documentation into a structured, submission-ready chargeback rebuttal packet — including an evidence narrative, supporting-documents checklist, and a merchant dispute response letter — so the shop can respond to credit-card chargebacks without hiring outside help or missing the bank's 7–30 day response window. | ~45 min/dispute |
...
| Emissions Service-Info & Tool Access Request Documenter | When a manufacturer, dealer, or OEM support line won't hand over the emissions-related service information, diagnostic tool access, training material, or reprogramming capability a shop needs — most often a DEF/SCR fault or an emissions-related reflash — this skill drafts the written request citing the shop's actual current federal footing, then escalates unresolved requests into a documented NASTF Service Information Request and, if needed, an owner/counsel escalation note. | ~20 min per request + escalation packet, if needed |
| Parts & Labor Matrix Optimizer | Audit the shop's current parts-markup matrix and labor-rate matrix, compare them against 60–90 days of actual closed repair-order data, and produce a recommended replacement matrix with explicit tier adjustments, projected gross-profit impact, customer-perception risk flags, and a 30-day rollout plan. | ~3 hr/quarter strategy work + ongoing GP lift on every RO that prices through the new matrix |
...
**Total time saved per use: ~685+ minutes across all skills.**
